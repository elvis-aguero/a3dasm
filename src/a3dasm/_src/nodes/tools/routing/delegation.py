"""Delegation-family tool closures: Delegate/Wait/GetStatus/CancelDelegation/
Reply/FollowUp/Confer/RecallHistory. Built per-node via
build_delegation_closures(node) — the node is passed in so closures reach its
state. Extracted verbatim from the former build_routing_tools()."""
...
def build_delegation_closures(node) -> dict:
    ...
    return {
        ...
        "Reply": Reply,
        "FollowUp": FollowUp,
        **({"RecallHistory": RecallHistory} if node._delegation_log is not None else {}),
        "GetStatus": GetStatus,
        "CancelDelegation": CancelDelegation,
        "Confer": _orchestrator_confer,
    }
