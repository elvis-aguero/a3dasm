...
- [ ] **#29** Per-delegation Write-sandbox permanently stuck to the FIRST delegation, for every OpenAI-compatible-backend worker — *open, found 2026-09-03 running MathExpert on Ollama (Qwen3.8:27b)*. Root cause: `OpenAICompatibleAdapter.copy()` (`backends/openai_compatible.py:677`) deliberately returns `self` ("concurrent delegations share this adapter instance"), and `_invoke_once()` (line ~739) does `if self._agent is None: self._agent = self._build_agent()` — the LangGraph agent, and its `_build_tools()`-baked `StructuredTool` wrapping of `closure_tools["Write"]`, is built ONCE and cached forever. `nodes/tools/routing.py`'s per-delegation sandbox setup (`worker.closure_tools["Write"] = node._wrap_closure(_sandboxed_write, target)`, ~line 1098) reassigns the dict entry fresh on every delegation, but for a worker on this adapter family the already-built agent's tool list never re-reads that dict — the reassignment is inert. Effect: every delegation to the SAME node past the first gets the FIRST delegation's sandbox (silently, no error at setup time — only surfaces when the write path itself lands outside the stale sandbox and the runtime's own write check rejects it, so on a run where writes always resolve inside the stale D001 folder this could go completely unnoticed). Confirmed via 3 real `_sandboxed_write` `ERROR_RETURN` diagnostics in `mathexpert_kinematic_matching_test` run `20260903T163807`: two errors reference `.../delegations/D001` while the actual live delegation was D002 and D004 respectively; a third shows a related but distinct manifestation (candidate path correctly under D004, but the printed sandbox root is the bare `delegations/` parent, one level too shallow). NOT specific to MathExpert — any repeatedly-delegated worker on Ollama/vLLM/OpenRouter has this. Judgment call, not fixed here: rebuild the agent per delegation for this backend family (defeats the "shared session" design `copy()`'s docstring states on purpose), or make the sandbox check read `closure_tools`/delegation context live at call time instead of baking a fixed callable into the tool list at first-build. Either is a real design change, not a one-line patch.
