...
- [ ] **#28** `nbformat.write` crashes with `AttributeError: outputs` at final notebook stamping — *open, found 2026-09-03, run `mathexpert_kinematic_matching_test/20260903T130138`* (cost: $1.84, 0 evals, ended FAILED). Traceback: `agent_runtime.py:1074` (`execute()`) → `nbformat.write(nb, str(nb_path))` → `nbjson.py:58 split_lines` iterates `cell.outputs` unconditionally → `AttributeError: outputs` on some cell in the notebook. `nbformat.v4.new_code_cell` (used by `AddPipelineCell`, `routing.py:2914`) normally initializes `outputs: []` by default, so some cell in this run's `pipeline.ipynb` was NOT constructed through the normal `new_code_cell`/`new_markdown_cell` path — root cause not yet traced to which cell/call produced it. Downstream of #27 (the strategizer wrote a pipeline.ipynb here despite `PROBLEM_STATEMENT.md` saying not to); independent of MathExpert itself. Needs a repro before fixing.
